...
import a from 'url-regex-safe'
let urlRegex = a({ strict: false })
import PhoneNumber from 'awesome-phonenumber'
import terminalImage from 'terminal-image'
import chalk from 'chalk' //, { redBright, black, magenta, green, yellow, blueBright, red, gray } from 'chalk'
...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(`
${chalk.redBright('%s')} ${chalk.bgYellow.inverse.black('%s')} ${chalk.bgGreen.inverse.black('%s')} ${chalk.magenta('%s [%s %sB]')}
${chalk.green('%s')} ${chalk.yellow('%s%s')} ${chalk.blueBright('to')} ${chalk.green('%s')} ${chalk.bgYellow.inverse.black('%s')}
`.trim(),
    me + ' ~' + conn.user.name,
